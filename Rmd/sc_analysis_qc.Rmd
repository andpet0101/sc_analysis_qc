...
<!-- general configuration chunk; should always be first and should not be cached -->
```{r general_configuration, include=show_code,echo=show_code,cache=F,tidy=T}
...
# source directory
if('sourcedir' %in% names(params)){
  if(!file.exists(params$sourcedir))stop("Specified source directory does not exist!")
  sourcedir = params$sourcedir
}else{
  stop("Please specify a source directory!")
}
...
# figures directory for individual plots
pdfdir = file.path(working_dir,'figures');
dir.create(pdfdir, showWarnings = FALSE)

# data directory for tables etc.
datadir = file.path(working_dir,'data')
...
rmarkdown_dir = file.path(sourcedir,'Rmd/')
opts_knit$set(child.path=rmarkdown_dir)

# base path for additional R files to include
r_include_dir = file.path(sourcedir,"R/")

# this is a (strange) hack to inject a new figure environment into latex
#
# the code is written into the file custom_figure_environment.tex which is then included by pandoc into the tex file
...
writeLines(tex_lines,file.path(workingdir,'custom_figure_environment.tex'))
```

<!-- parameter configuration chunk -->
```{r parameter_configuration, opts.label='code_chunk'}
############ configuration ############ 
...
if(!'annotation' %in% names(params))stop("Missing a annotation version ('annotation') entry in the 'params' section! Can be an Ensembl version or a path to a file with custom annotations.")
...
# source neccessary functions
source(file.path(r_include_dir,'setup_sce_functions.R'))

# create sce object
sce = create_sce_from_countsdata(countsdata) 

# parse plate information (if available)
